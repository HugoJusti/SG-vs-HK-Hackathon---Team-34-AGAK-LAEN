...
Note: These variables values are subjected to change based on a Machine Learning (ML) model we test and market conditions, All these variables also don't need to pass for the bot to Enter or Exit (We weight the variables based on how important they are). 
...
- **Momentum > 0.0181** (10-period Rate of Change): price must be trending upward
- **Price > EMA20**: price must be above the 20-period exponential moving average

In addition, **at least 2 of the following 4** must also pass:
- **HMM bullish confidence > 0.59**: the Hidden Markov Model classifies current regime as bullish
- **Annualised rolling volatility < 0.66**: market is not too volatile to trade
- **Volume z-score |z| < 4.28**: no abnormal volume spike (avoids manipulation candles)
- **Bid-ask spread < 0.05%**: market is liquid enough for clean execution

### Exit Conditions:
Positions are exited under two hard rules (whichever triggers first):
- **Stop Loss**: close price drops 3% below entry price
- **Take Profit**: close price rises 3% above entry price

There are no indicator-based exits — every trade is purely bounded by the 3% SL/TP brackets.
...
- 3% SL/TP is appropriate for short-term 5-minute timeframe trades in a 7-day competition
